local OutputBuffer = require("tests/OutputBuffer")
...
function Tester.new(testLibrary, testModule, testData, unsafeMode, failThrowsError)
    local self = {}
    
    self.testModule = testModule
    self.testData = testData
    self.unsafeMode = unsafeMode or false
    self.failThrowsError = failThrowsError or false
...
function Tester:runTest(test, call)
    ...
    if self.unsafeMode then
        result, reason = test.test()
        success = true
        if reason then
            print(test.DisplayName, reason)
        end
    else
