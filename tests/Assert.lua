...
function Assert.ApproxEquals(a, b, epsilon)
	if not epsilon then warn("Epsilon not passed!") epsilon = 1e-6 end
	local aIsNaN = a ~= a
	local bIsNaN = b ~= b
...
function Assert.VectorsApproxEqual(a: Vector3, b: Vector3, epsilon: number)
...
function Assert.CFramesApproxEqual(
	a: CFrame, b: CFrame, epsilon: number, includePosition: boolean
)
